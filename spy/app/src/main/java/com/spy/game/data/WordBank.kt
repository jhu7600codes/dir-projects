 ...
 *    has run.
 *  - a larger (~3,100-word) general Russian word list bundled as
 ...
 *    Leeds University Russian corpus's 10,000-most-common-words frequency
 *    list (CC BY 2.5 -- see README), run through a real morphological
 *    analyzer (pymorphy3), and kept only if its single most likely parse is
 *    *unambiguously* a common noun in the nominative case -- no adjectives,
 *    no verbs, no other part of speech (including words that parse as a
 *    noun but tie for top score with a non-noun reading, like "мертвый" =
 *    "dead [person]" vs. "dead"), and no personal names (pymorphy's
 *    Name/Surn/Patr grammemes, plus a manual blocklist for names its
 *    dictionary under-tags) or org/brand abbreviations. On top of that,
 *    words over 10 characters and a short list of vulgar/offensive terms
 *    are dropped, so what's left reads as simple, everyday, easy-to-act-out
 *    nouns rather than bureaucratic/abstract vocabulary ("целесообразность",
 *    "функциональность") that a frequency list alone still lets through.
 */
object WordBank {
