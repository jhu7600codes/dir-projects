...
// static, not just t fun: statically linking against a real ALC (like this
// vitasdk openal-soft build) puts alcGetProcAddress()-loaded pointers of
// the same name in the same executable as the real functions, so these
// need internal linkage to avoid colliding with them at link time
