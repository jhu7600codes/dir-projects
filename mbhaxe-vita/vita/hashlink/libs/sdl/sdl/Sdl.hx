...
class Sdl {
	...
	static function eventLoop(e:Dynamic)
		return false;

	static function hintValue(name:hl.Bytes, value:hl.Bytes)
		return false;
	...
	public static function getScreenWidth(?win:sdl.Window):Int {
		return if (win == null) get_screen_width(); else get_screen_width_of_window(@:privateAccess win.win);
	}

	public static function getScreenHeight(?win:sdl.Window):Int {
		return if (win == null) get_screen_height(); else get_screen_height_of_window(@:privateAccess win.win);
	}
	...
	public static function getDevices() {
		...
		for (v in arr) {
			...
			if (dnames.exists(d) || StringTools.startsWith(d, "RDP") /* RemoteDesktop */)
				continue;
			dnames.set(d, true);
			a.push(d);
		}
