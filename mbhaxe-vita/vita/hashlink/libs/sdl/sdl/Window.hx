...
class Window {
	...
	public function new(title:String, width:Int, height:Int, x:Int = SDL_WINDOWPOS_CENTERED, y:Int = SDL_WINDOWPOS_CENTERED,
			sdlFlags:Int = SDL_WINDOW_SHOWN | SDL_WINDOW_RESIZABLE) {
		while (true) {
		...
		try {
			...
			if (isOpenGLES) {
				...
				if (reg.match(v)) {
					...
					if (shaderVersion < minVer)
						shaderVersion = minVer;
				}
			}
			...
			if (GL.getShaderParameter(vertex, GL.COMPILE_STATUS) != 1)
				throw "Failed to compile VS (" + GL.getShaderInfoLog(vertex) + ")";

			var fragment = GL.createShader(GL.FRAGMENT_SHADER);
			if (isOpenGLES)
				GL.shaderSource(fragment, [
					"#version " + shaderVersion,
					"lowp vec4 color; void main() { color = vec4(1.0); }"
				].join("\n"));
			else
				GL.shaderSource(fragment, [
					"#version " + shaderVersion,
			...
			if (GL.getShaderParameter(fragment, GL.COMPILE_STATUS) != 1)
				throw "Failed to compile FS (" + GL.getShaderInfoLog(fragment) + ")";
			...
			if (GL.getProgramParameter(p, GL.LINK_STATUS) != 1)
				throw "Failed to link (" + GL.getProgramInfoLog(p) + ")";
	...
	function set_displayMode(mode) {
		if (winSetFullscreen(win, mode)) {
			displayMode = mode;
			if (mode == Fullscreen) {
				try @:privateAccess sdl.Window.winSetDisplayMode(win, displaySetting.width, displaySetting.height, displaySetting.framerate)
				catch (_) {}
			}
		}
		return displayMode;
